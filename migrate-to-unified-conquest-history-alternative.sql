-- Script alternativo para migrar para tabela unificada conquest_history
-- Execute este SQL no SQL Editor do Supabase (sem PostGIS)
...
ADD COLUMN IF NOT EXISTS polygon_coords TEXT, -- Usar TEXT em vez de JSONB
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Constraint única já existe para territory_id';
    END IF;
END $$;
...
SELECT 
  ...
  area as area_lost,
  polygon::text as polygon_coords, -- Converter geometry para TEXT
  created_at,
  'active' as status
FROM territories
